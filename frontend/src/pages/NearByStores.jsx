...
const StoreCard = ({ store }) => {
    const { storeImage } = useContext(ShopContext);
    return (
        <div className="border rounded-xl p-5 shadow-md hover:shadow-xl transition duration-300 bg-white w-full max-w-sm mx-auto flex flex-col">
            ...
            <div className="mt-4 flex-1 flex flex-col justify-between">
                <div>
                    ...
                    <p className="text-gray-600 text-sm mt-1">📍 {store.address}  away</p>
                    <p className="text-gray-600 text-sm mt-1"><span className="font-bold">Village:</span> {store.village}</p>
                    <p className="text-gray-600 text-sm mt-1"><span className="font-bold">City:</span> {store.city}</p>
                    ...
                    <p className="text-sm">🚚 Home Delivery Available Pickup Only</p>
        ...
        <div className="p-6 bg-gray-100 min-h-screen">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {stores.map((store,index) => (
                    <StoreCard key={store._id || index} store={store} />
                ))}
            </div>
        </div>
    );
